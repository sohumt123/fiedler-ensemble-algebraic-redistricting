...
from gerrydetect.data_mggg import load_mggg_nc
...
log = logging.getLogger("run_mst_nc")
...
def render_nc_panel(
    ...
    fig.suptitle(
        f"North Carolina (real data, with MST diameter): 2016 CD plan vs. "
        f"{len(ensemble_df)}-plan MCMC ensemble (k={n_seats}, n={len(gdf)} VTDs)",
...
def main() -> None:
    ...
    t0 = time.time()
    log.info("Loading NC MGGG shapefile ...")
    gdf = load_mggg_nc(REPO_ROOT / "data/raw/nc_mggg")
    ...
    log.info("Computing metrics on %d ensemble plans (includes MST) ...", len(pooled))
    t0 = time.time()
    ensemble_rows = []
    ...
    log.info("NC composite severity (5 metrics): %.3f", severity)

    long_df = pd.DataFrame(rows_out)
    long_df.to_csv(TAB_DIR / "nc_real_long.csv", index=False)
    log.info("Saved nc_real_long.csv")
    ...
    log.info("  Severity:      %.3f", severity)

    fig_path = REPO_ROOT / "output" / "figures" / "real_nc" / "nc_real_panel.png"
    docs_path = REPO_ROOT / "docs" / "figures" / "real_nc" / "nc_real_panel.png"
    render_nc_panel(
    ...
    log.info("Figure updated at %s", docs_path)

    log.info("Done. NC MST analysis complete.")
