...
from gerrydetect.data_mggg import load_mggg_md
...
log = logging.getLogger("run_mst_md")
...
def render_md_panel(
    ...
    fig.suptitle(
        f"Maryland (real data, with MST diameter): 2011 CD plan vs. "
...
def main() -> None:
    ...
    t0 = time.time()
    log.info("Loading MD MGGG shapefile ...")
    gdf = load_mggg_md(REPO_ROOT / "data/raw/md_mggg")
    ...
    log.info("MD composite severity (5 metrics): %.3f", severity)

    long_df = pd.DataFrame(rows_out)
    long_df.to_csv(TAB_DIR / "md_real_long.csv", index=False)
    log.info("Saved md_real_long.csv")
    ...
    fig_path = REPO_ROOT / "output" / "figures" / "real_md" / "md_real_panel.png"
    docs_path = REPO_ROOT / "docs" / "figures" / "real_md" / "md_real_panel.png"
    render_md_panel(
    ...
    log.info("Done. MD MST analysis complete.")
