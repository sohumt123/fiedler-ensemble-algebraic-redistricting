...
from gerrydetect.data_mggg import load_mggg_wi
...
log = logging.getLogger("run_mst_wi")
...
def render_wi_panel(
    ...
    fig.suptitle(
        f"Wisconsin (real data, with MST diameter): 2011 CD plan vs. "
...
def main() -> None:
    ...
    t0 = time.time()
    log.info("Loading WI MGGG shapefile ...")
    gdf = load_mggg_wi(REPO_ROOT / "data/raw/wi_mggg")
    ...
    log.info("WI composite severity (5 metrics): %.3f", severity)

    long_df = pd.DataFrame(rows_out)
    long_df.to_csv(TAB_DIR / "wi_real_long.csv", index=False)
    log.info("Saved wi_real_long.csv")
    ...
    fig_path = REPO_ROOT / "output" / "figures" / "real_wi" / "wi_real_panel.png"
    docs_path = REPO_ROOT / "docs" / "figures" / "real_wi" / "wi_real_panel.png"
    render_wi_panel(
    ...
    log.info("Done. WI MST analysis complete.")
